...
uint8_t history_header[12] = {0xFE, 0xFE, 0x00, 0x23, 0x12, 0x02, 0x02, 0x01, 0x00, 0x00, 0x01, 0x00}; //history_header[3] = size, history_header[10] = payload number
...
void loop()
{
  ...
  if (!strncmp(buf, request, 10))
  {
    ...
    for (size_t i = 0; i < 24; i++)
      history[i + 12] = payload[i];

    for (size_t i = 0; i < 2; i++)
      history[i + 36] = history_tail[i];

    Serial.write("receive history request");
    bleSerial.write(history, 38);
  }
  else
  {
